...
class Launcher {
  constructor(configFile, argv = {}, isWatchMode = false) {
    ...
    const Runner = (0, _utils.initialisePlugin)(config.runner, 'runner');
    this.runner = new Runner(configFile, config);
    this.interface = new _interface.default(config, specs, totalWorkerCnt, isWatchMode);
  ...
  runSpecs() {
    ...
    while (this.getNumberOfRunningInstances() < config.maxInstances) {
      ...
      this.startInstance(specs.files, schedulableCaps[0].caps, schedulableCaps[0].cid, schedulableCaps[0].seleniumServer, specs.rid, specs.retries);
  ...
  endHandler({
    ...
    if (!passed && retries > 0) {
      this.schedule[parseInt(cid)].specs.push({
        files: specs,
        retries: retries - 1,
        rid: cid
      });
